...
void raytraceImage(vector<unsigned char>& image, Camera& camera, Scene& scene)
{
	...
	for_each(execution:: par_unseq, px_indices.begin(), px_indices.end(), [&](int Px) {
		...
		// Definim les variables locals de cada thread - Si no, passa el que es coneix com a condició de carrera, que els threads sobreescriuen variables compartides amb altres threads donant lloc a errors.
